...
cdat   <- rawdat %>% mutate(Date = format(date, "%m/%d"), 
                            Topics = topic, Materials = notes,
                            Prepare = before, Together=during,
                            HW = after, Wk=Week) %>% 
              select(Wk, Date, Topics, Materials, Prepare, Together, HW, topic, notes, before, during, after)
...
out <- select(cdat, Wk, Date, Topics,  Materials, Prepare, Together, HW )
...
knitr::kable(out, align="l", escape=FALSE) %>% 
  ...
  column_spec(3, width="10%") %>% column_spec(4, width="10%") %>%
  column_spec(5, width="20%") %>% column_spec(6, width="20%") %>% column_spec(7, width="20%")
```
